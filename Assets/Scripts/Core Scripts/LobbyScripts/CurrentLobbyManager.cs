...
public class CurrentLobbyManager : MonoBehaviour
{
    ...
    public async Task SpawnPlayerPrefabs(Lobby l, LobbyManager lob)
    {
        ...
        foreach (var vPlayer in l.Players)
        {
            ...
            if (vPlayer.Id == l.HostId || l.HostId != AuthenticationService.Instance.PlayerId) pps.DisableKickForHost();
        }
    ...
    {
        if (playerCollectionPanel.GetComponentInChildren<Transform>().childCount == 0) return;
        foreach (Transform t in playerCollectionPanel.transform) Destroy(t.gameObject);
    }
}